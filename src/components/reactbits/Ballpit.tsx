...
function onTouchStart(e: TouchEvent) {
  if (e.touches.length > 0) {
    // Local tweak (mobile): no preventDefault — see the onTouchMove note. The
    // canvas uses touch-action: pan-y so the page still scrolls past the pit.
...
function onTouchMove(e: TouchEvent) {
  if (e.touches.length > 0) {
    // Local tweak (mobile): no preventDefault. The upstream code called it on
    // every touchmove on document.body, which trapped the whole page's scroll on
    // phones the moment the pit mounted. With the canvas set to touch-action:
    // pan-y, a vertical drag scrolls the page (and the marbles still follow the
    // finger) while a horizontal drag swirls the pit without scrolling.
    pointerPosition.set(e.touches[0].clientX, e.touches[0].clientY);
    for (const [elem, data] of pointerMap) {
      const rect = elem.getBoundingClientRect();
      updatePointerData(data, rect);
...
// (Removed unused scratch vectors/destructure from the upstream snippet to
// satisfy this project's noUnusedLocals.)
...
function createBallpit(canvas: HTMLCanvasElement, config: any = {}): CreateBallpitReturn {
  ...
  // Local tweak (mobile perf): cap the device-pixel-ratio at 2 so high-DPR phones
  // (often 3x) don't render this physics sim at ~9x the pixels and overheat/jank.
  ...
  // Local tweak (mobile): pan-y (not 'none') so a vertical finger-drag over the
  // pit still scrolls the page; horizontal drags swirl the marbles. Paired with
  // dropping preventDefault in the touch handlers above so scrolling is never
  // trapped.
...
const Ballpit: React.FC<BallpitProps> = ({ className = '', followCursor = true, ...props }) => {
  // Local tweak: render a container and create a FRESH <canvas> per mount inside
  // the effect (removing it on cleanup), instead of reusing one React-owned
  // <canvas>. The upstream version reused a single canvas, and dispose()'s
  // forceContextLoss() poisons it — so React 18 StrictMode's dev double-mount
  // left the second mount with a null WebGL context (blank pit). A new canvas
  // each mount gets a clean context and is StrictMode-safe.
